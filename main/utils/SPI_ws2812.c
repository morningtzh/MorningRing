...
void led_strip_update(uint8_t *buffer) {
    ...
    for (i = 0; i < LED_MAX_NBER_LEDS; i++) {
        uint8_t r = buffer[i * 3];// Data you want to write to each LEDs, I'm my case it's 95 RGB x 3 color
        uint8_t g = buffer[i * 3 + 1];// Data you want to write to each LEDs, I'm my case it's 95 RGB x 3 color
        uint8_t b = buffer[i * 3 + 2];// Data you want to write to each LEDs, I'm my case it's 95 RGB x 3 color
